...
public class UserVariableHandler implements MutableVariables {
                ...
                if (customHandlers.containsKey(handlerName)) {
                    ...
                    if (!(handler instanceof MutableVariables)) {
                        throw new RuntimeException(
                                "Not a mutable variable: " + upgrade + " (tried to upgrade to level " + level + ")");
                    }
                    ((MutableVariables) handler).setVariable(varName, level);
                } else {
                    throw new RuntimeException(
                            "Could not update variable " + upgrade + " to " + level + " as no handler");
                }
            }
        } else {
    ...
    @Override
    public MutableVariables copyVariables() {
        return new MapVariables(variables);
    }
